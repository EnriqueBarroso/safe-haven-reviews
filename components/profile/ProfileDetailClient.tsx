...
import {
  ...
  ShieldCheck, Download, Link as LinkIcon, CheckCircle2, MessageSquare,
} from "lucide-react"
...
export function ProfileDetailClient({
  ...
  useEffect(() => {
    // profileUrl solo existe en el browser (BBCode sharing)
    setProfileUrl(window.location.href)
    // Sesión: solo se necesita para mostrar/ocultar el botón "Responder"
          ...
          <div className="space-y-6 pt-4">
            <div className="flex items-center justify-between border-b pb-4">
              ...
              <span className="text-muted-foreground">
                {reviewTree.length + questionTree.length} aportaciones
              </span>
            </div>
            ...
            <Tabs defaultValue={defaultTab} className="w-full">
              <TabsList className="grid w-full grid-cols-2 max-w-md mb-8 bg-secondary/50">
                <TabsTrigger
                  value="reviews"
                  className="flex items-center gap-2 data-[state=active]:bg-background data-[state=active]:text-primary data-[state=active]:shadow-sm"
                >
                  <StarIcon className="h-4 w-4" /> Reseñas ({reviewTree.length})
                </TabsTrigger>
                <TabsTrigger
                  value="forum"
                  className="flex items-center gap-2 data-[state=active]:bg-amber-500/10 data-[state=active]:text-amber-600 data-[state=active]:shadow-sm"
                >
                  <MessageSquare className="h-4 w-4" /> Foro ({questionTree.length})
                </TabsTrigger>
              </TabsList>
            ...
            <CardContent className="space-y-5 pt-6">
              ...
              <div className="pt-6 border-t">
                <Button className="w-full h-12 text-base font-bold" size="lg" asChild>
                  <Link
                    href={`/submit-review?profileId=${profile.id}&name=${encodeURIComponent(profile.name)}&city=${encodeURIComponent(profile.city)}`}
                  >
