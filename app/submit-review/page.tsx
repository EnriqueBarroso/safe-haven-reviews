...
import { Loader2, Save, AlertCircle, MessageSquare, Star, ArrowLeft, Lock, ImageIcon } from "lucide-react"
...
// ─── Generador de slug (mismo algoritmo que el trigger de Postgres) ──
function generateSlug(name: string, city: string): string {
  ...
  return str
    ...
    .replace(/[^a-z0-9\-]/g, "-")   // reemplaza caracteres no válidos
...
export default function SubmitReviewPage() {
  ...
  const handlePublish = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 1. Generamos el slug aquí — así siempre tenemos el valor
      // independientemente de si el perfil es nuevo o ya existía
      ...
      if (existing) {
        ...
        // El trigger de Postgres generará el slug automáticamente,
        // pero lo enviamos también por si el trigger no está activo
      ...
      // 4. Insertar reseña o pregunta
      const payload = postType === "review"
        ? {
        ...
        : {
            profile_id: profileId, user_id: session.user.id, type: "question",
            alias: userProfile.alias, avatar_url: userProfile.avatarUrl,
            details: formData.details,
          }
      ...
      // 6. Redirect usando el slug — siempre definido porque lo generamos
      // en el paso 1, independientemente de si el perfil era nuevo o existente
  ...
  return (
    <div className="container mx-auto py-10 px-4 max-w-3xl">
      ...
      <div className="grid grid-cols-2 gap-3 mb-6">
        <button type="button" onClick={() => setPostType("review")}
          ...
          <span className="text-xs text-center opacity-70">Comparte tu experiencia con la comunidad</span>
        </button>
        <button type="button" onClick={() => setPostType("question")}
          ...
          <span className="text-xs text-center opacity-70">Consulta algo a la comunidad</span>
        </button>
      </div>
          ...
          <CardContent className="pt-8">
            <form onSubmit={handlePublish} className="space-y-8">
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input value={formData.city}
                      ...
                      readOnly={isReadOnly}
                      className={isReadOnly ? "bg-muted cursor-not-allowed" : ""}
                      placeholder="Ej: Barcelona" required />
                  </div>
                </div>
                <div className="space-y-2">
                  <Label>
              ...
              <div className="space-y-4 border-t pt-8">
                ...
                <Textarea
                  placeholder="Ej: ¿Las fotos son reales? ¿Alguien ha estado recientemente?"
                ...
                <div className="p-3 rounded-lg bg-amber-500/10 border border-amber-200/50 text-xs text-amber-700 dark:text-amber-400">
                  💬 Tu pregunta será visible para toda la comunidad.
                </div>
              </div>

              <FirmaUsuario alias={userProfile.alias} avatarUrl={userProfile.avatarUrl} variant="question" />
